...
void scalar_shallowwater(Data **data, Map *smap, Config *param, int irank, int nrank, int kk)
{
    ...
    for (ii = 0; ii < param->n2ci; ii++)
    {
        ...
        if ((*data)->rain_sum[0] > param->min_dept & (*data)->Vflux[ii] > 0.0)
        {
            s_min[ii] = s_min[ii] * ((*data)->Vflux[ii] - (*data)->rain_sum[0]*(*data)->Asz[ii]) / (*data)->Vflux[ii];
        }
        if ((*data)->rain[0] == 0.0 & (*data)->evap[ii] > 0.0 & (*data)->Vflux[ii] > 0.0)
        {
            s_max[ii] = s_max[ii] * ((*data)->Vflux[ii] + (*data)->evap[ii]*param->dt*(*data)->Asz[ii]) / (*data)->Vflux[ii];
        }
...
void scalar_groundwater(Data **data, Map *gmap, Config *param, int irank, int nrank, int kk)
{
    ...
    for (ii = 0; ii < param->n3ci; ii++)
    {
        ...
        if (gmap->istop[ii] == 1)
        {
            if (param->sim_shallowwater == 1 & (*data)->dept[gmap->top2d[ii]] > 0.0)
            {
            ...
            {
                // limiter not applied on dry surface
            }
        }
        else
        {
...
double dispersive_flux(Data **data, Map *gmap, Config *param, int icell, int kk, char* axis)
{
    ...
    if (strcmp(axis, "x") == 0 & gmap->actv[icell] == 1)
    {
        ...
        if ((*data)->Kz[icell] > 0 & gmap->actv[gmap->icjckP[icell]] == 1 & gmap->istop[icell] == 0)
        {
